...
router.post('/in', async (req, res) => {
    ...
    try {
        ...
        // Insert or update data into the item and transaction_detail tables
        ...
        const itemUpdateQuery = `
            UPDATE item 
            SET itemqty = itemqty + ?
            WHERE itemname = ? AND expdate = ? AND categoryid = ?
        `;
        ...
        for (const item of items) {
            ...
            const [existingItem] = await db.query(
                ...
                [itemName, expDateValue, categoryId]
            );

            if (existingItem.length > 0) {
                // 2 If found, update the existing item quantity
                await db.query(itemUpdateQuery, [itemQty, itemName, expDateValue, categoryId]);
                const existingItemId = existingItem[0].itemid;

                // 3 Insert into transaction_detail table with existing itemid
                await db.query(transactionDetailInsertQuery, [transactionId, existingItemId, itemQty]);
            } else {
                // 4 If not found, insert as a new item
                ...
                // 5️ Insert into transaction_detail table with new itemid
